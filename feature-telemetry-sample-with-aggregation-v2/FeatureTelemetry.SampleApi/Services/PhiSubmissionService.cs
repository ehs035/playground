...
public sealed class PhiSubmissionService : IPhiSubmissionService
{
    public async Task<string> SubmitAsync(SubmitPhiFormRequest req, CancellationToken ct)
    {
        ...
        if (req.MemberId == "timeout") throw new TimeoutException("PHI submission timed out.");
        if (req.MemberId == "error") throw new InvalidOperationException("Submission failed.");
