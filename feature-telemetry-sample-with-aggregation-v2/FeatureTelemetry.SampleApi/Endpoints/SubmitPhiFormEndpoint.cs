...
public sealed class SubmitPhiFormEndpoint : TelemetryEndpoint<SubmitPhiFormRequest, SubmitPhiFormResponse>
{
    ...
    protected override Task OnBeforeHandleAsync(SubmitPhiFormRequest req, CancellationToken ct)
    {
        _phiSubmissionService = Resolve<IPhiSubmissionService>();
        return Task.CompletedTask;
    }

    protected override async Task<SubmitPhiFormResponse> ExecuteFeatureAsync(
        SubmitPhiFormRequest req,
        CancellationToken ct)
    {
        var id = await _phiSubmissionService.SubmitAsync(req, ct);
